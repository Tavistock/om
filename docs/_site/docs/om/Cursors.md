...
You can create sub-cursors from cursors by just calling `get` or `get-in` on them (works only during the render phase). There's a gotcha: if the value returned by `get` is a map or a vector, you’ll get a sub-cursor pointing to it. If it's a primitive value or nil, you'll get the primitive value (or nil), not a cursor.
...
Updating values that *might* be nil requires some extra care. You can't pass a sub-cursor (see previous section) to `transact!` in these cases, because the "cursor" itself will also be nil. You can work around this by passing a parent cursor, and using the longer form of `transact!` (see the API docs) which takes a target key as an additional argument. 
